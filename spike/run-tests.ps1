...
if (-not (Test-Path (Join-Path $ScriptDir 'baseline-local.Export.wacomxs'))) {
    ...
    Write-Host "Export it first:"
    Write-Host "  & 'C:\Program Files\Tablet\Wacom\PrefUtil.exe' /export '$ScriptDir\baseline-local.Export.wacomxs'"
    Write-Host "Then click OK in the PrefUtil dialog and re-run this script."
    exit 1
}
...
Write-Host ">> Record in SPIKE-RESULTS.md: exit code, stylus restriction (YES/NO/PARTIAL)."
Write-Host ">> Press Enter to continue to TC-04 (latency measurement)..." ; Read-Host
Write-Host ""
...
Write-Host ">> Press Enter to continue to TC-05 (reset to full-screen)..." ; Read-Host
Write-Host ""
...
Write-Host "  3. Return to the GSD plan with: 'passed', 'partial [description]', or 'failed [description]'"
